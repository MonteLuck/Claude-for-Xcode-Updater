...
3. Downloads the requested version from the Anthropic Claude Code releases CDN
4. Verifies the binary reports the requested version
5. Computes a SHA-512 checksum and writes `Info.plist`
6. Backs up the existing `claude` binary and `Info.plist`
7. Installs the requested version into the Xcode agent directory
